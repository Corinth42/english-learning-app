...
def word_learning_tab(df, word_master):
    ...
    <div class="sentence-card">
        ...
        <h4>英文:</h4>
        <p style="font-size: 1.2rem; line-height: 1.6;">{current_sentence['sentence_content_en']}</p>
    </div>
    ''', unsafe_allow_html=True)
    ...
    if st.session_state.show_translation:
        ...
        <div class="translation-card">
            <h4>🇯🇵 日本語訳:</h4>
            <p style="font-size: 1.1rem;">{current_sentence['translated_sentence']}</p>
        </div>
        ''', unsafe_allow_html=True)
...
def shadowing_tab():
    ...
    if not st.session_state.get('gemini_api_key'):
        ...
        api_key = st.text_input("Google AI APIキー", type="password", help="Google AI Studioで取得したAPIキーを入力")
